...
namespace YouAskedForIt.Patches
{
    ...
    {
        static readonly Type TARGET_TYPE = typeof(MainMenu);
        const bool IS_ORIGINAL_LAMBDA_BODY = false;
        const int LAMBDA_BODY_INDEX = 0;
        const string TARGET_METHOD_NAME = "Setup";
        ...
        {
            null,
            typeof(MainMenu_Patch).GetMethod("GetPracticeButtonText", BindingFlags.NonPublic | BindingFlags.Static)
        };

        static string GetPracticeButtonText(GlobalLocalisation globalLocalisation, string key)
        {
            if (Main.PrefManager.Get<bool>(Main.REHEARSAL_TIME_ID))
                return Main.CUSTOM_PRACTICE_MODE_TEXT;
            return globalLocalisation[key];
        }
